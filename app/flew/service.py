...
from app.flew.models import SubscriptionSource, AggregatedConfig
...
class FlewService:
    ...
    def __init__(self):
        self.runtime_file = "data/flew_runtime.json"
        self._load_runtime_settings()

    def _load_runtime_settings(self):
        """Загружает runtime-настройки Flew."""
        try:
            ...
            if isinstance(target_ips, list):
                ...
                if normalized:
                    app_config.FLEW_TARGET_CHECK_IPS = normalized
    ...
    def _save_runtime_settings(self):
        try:
            ...
            payload = {
                "target_ips": list(app_config.FLEW_TARGET_CHECK_IPS),
    ...
    def get_target_ips(self) -> List[str]:
        return list(app_config.FLEW_TARGET_CHECK_IPS)
    ...
    def set_target_ips(self, target_ips: List[str]) -> List[str]:
        ...
        app_config.FLEW_TARGET_CHECK_IPS = unique
    ...
    async def update_subscription(self) -> dict:
        ...
        # Синхронизация с Flew Core
        try:
            sync_result = flew_core_integration.sync_active_configs_to_core()
            logger.info(f"Flew Core sync result: {sync_result}")
            # NOTE: do not cleanup hosts globally here.
            # Cleanup removed user-created and non-Flew hosts unexpectedly.
        except Exception as e:
    ...
    def get_stats(self) -> dict:
        ...
        # Include direct configs in global totals shown by Flew Statistics.
        ...
        stats["target_ips"] = app_config.FLEW_TARGET_CHECK_IPS
        stats["domain"] = app_config.FLEW_DOMAIN
        return stats


flew_service = FlewService()
