"""
Интеграция Flew с Flew Core
Автоматическое добавление проверенных конфигураций в Flew Core
"""
...
from app.flew.models import AggregatedConfig, DirectConfig
from app.flew.storage import storage
from app import db

logger = logging.getLogger(__name__)


class FlewCoreIntegration:
    """Сервис интеграции с Flew Core"""
    ...
    def config_to_proxy_host(self, config: AggregatedConfig) -> ProxyHostModify:
        """Конвертация конфигурации в ProxyHost для Flew Core"""
        return ProxyHostModify(
            remark=f"Flew-{config.protocol.upper()}-{config.server[:15]}",
    ...
    def direct_config_to_proxy_host(self, config: DirectConfig) -> ProxyHostModify:
        """Конвертация прямой конфигурации в ProxyHost для Flew Core"""
        return ProxyHostModify(
    ...
    def sync_active_configs_to_core(self) -> Dict:
        """Синхронизация активных конфигов с Flew Core"""
        try:
            ...
            logger.info(f"Flew Core sync complete: {synced_count} configs synced")
            ...
            logger.error(f"Flew Core integration failed: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def cleanup_inactive_hosts(self, active_configs: List[AggregatedConfig]) -> Dict:
        """Очистка неактивных хостов из Flew Core"""
        try:
    ...
    def sync_direct_config_to_core(self, config: DirectConfig) -> Dict:
        """Синхронизация прямой конфигурации с Flew Core"""
        try:
            # Direct configs should NOT be converted into Flew Core hosts.
            return {
                "status": "disabled",
            ...
            logger.info(f"Added direct config to Flew Core: {config.protocol}://{config.server}:{config.port}")
            ...
            logger.error(f"Failed to sync direct config to Flew Core: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def sync_all_direct_configs_to_core(self) -> Dict:
        """Синхронизация всех активных прямых конфигураций с Flew Core"""
        try:
...
flew_core_integration = FlewCoreIntegration()
