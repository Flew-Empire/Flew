...
from app.flew.models import UserPingStats, AggregatedConfig
...
class PingStatsService:
    ...
    def __init__(self):
        self.stats_file = "flew_ping_stats.json"
    ...
    def get_top_configs(self, configs: List[AggregatedConfig], limit: int = 10) -> List[AggregatedConfig]:
        ...
        if not app_config.FLEW_USE_DYNAMIC_FILTERING:
            return configs[:limit]
        
        # Оцениваем каждый конфиг
        scored_configs = []
        min_users = app_config.FLEW_MIN_USERS_FOR_STATS
