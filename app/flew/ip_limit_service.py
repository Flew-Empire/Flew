...
from app.utils.jwt import extract_subscription_token_from_url, get_subscription_payload
from config import XRAY_SUBSCRIPTION_PATH, FLEW_IP_ROTATION_WINDOW_SECONDS

_storage_file = "data/sub_ip_limits.json"
_storage_lock = threading.Lock()

WINDOW_SECONDS_DEFAULT = int(FLEW_IP_ROTATION_WINDOW_SECONDS or 0)
...
def extract_subscription_token(url: str) -> Optional[str]:
    return extract_subscription_token_from_url(url)
...
def get_unique_ip_limit_state_for_username(username: str) -> Dict[str, Any]:
    username = (username or "").strip()
    if not username:
        return {
            "limit": DEFAULT_UNIQUE_IP_LIMIT,
    ...
    with _storage_lock:
        data = _load_data()
        raw_entry = data.get("users", {}).get(username) or {}
    ...
    return {
        ...
        "disabled": disabled,
        "has_override": bool(disabled or has_custom_limit),
    }
