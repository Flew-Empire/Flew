...
Мониторинг использования внешних VPN серверов через Flew
"""
...
from config import FLEW_TRAFFIC_DB_PATH, FLEW_TRAFFIC_RETENTION_DAYS
...
class TrafficService:
    ...
    def __init__(self):
        self.db_path = FLEW_TRAFFIC_DB_PATH or "data/traffic_stats.db"
        self.retention_days = FLEW_TRAFFIC_RETENTION_DAYS or 0
        self._init_db()
    ...
    def get_admin_traffic_usage(self, admin_username: str, days: int = 30) -> Dict:
        """Получает статистику трафика для админа (интеграция с Flew Core)"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
        ...
        def reset_admin_external_traffic(self, admin_username: str) -> Dict:
            """Сброс внешнего трафика для админа (интеграция с Flew Core)"""
            try:
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
