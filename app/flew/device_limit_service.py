...
def _get_storage_file() -> str:
    env_path = (os.getenv("FLEW_DEVICE_LIMITS_FILE") or "").strip()
    if env_path:
        return env_path
...
def get_device_settings_state_for_username(username: str) -> Dict[str, Any]:
    username = _normalize_str(username)
    if not username:
        return {
    ...
    with _storage_lock:
        data = _load_data()
        raw_entry = (data.get("users") or {}).get(username) or {}
...
def set_device_settings_for_username(
    ...
    with _storage_lock:
        ...
        raw_entry = users.get(username) or {}
        entry = _normalize_entry(raw_entry)

        if limit is None and not unlimited:
            normalized_limit = DEFAULT_ALLOWED_DEVICES
            clear_override = True
        elif limit is None:
        ...
        if not clear_override:
            stored_entry["limit"] = int(entry["limit"])
            stored_entry["unlimited"] = bool(entry["unlimited"])

        users[username] = stored_entry
        data["users"] = users
        _save_data(data)
        ...
        return {
            ...
            "has_override": not clear_override,
        }
