...
const safeWriteText = async (text) => {
  if (
...
const safeReadText = async (promptMessage = "Paste JSON here") => {
  if (
    typeof navigator !== "undefined" &&
    typeof window !== "undefined" &&
    navigator.clipboard &&
...
createApp({
  setup() {
    ...
    const copyToClipboard = async () => {
      const success = await safeWriteText(generatedConfig.value);
    ...
    const importFromClipboard = async () => {
      try {
        const text = await safeReadText("Paste inbound JSON here");
        if (text === null) {
          return;
        }
    ...
    if (typeof window !== "undefined") {
      window.__ddsInboundGenerator = {
        getGeneratedConfig: () => generatedConfig.value,
        importConfig: (value) => {
