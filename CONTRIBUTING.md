# Contribute to Flew
Thanks for considering contributing to Flew!

## Questions

Please don't ask your questions in issues. Instead, use one of the following ways to ask:
- Ask on our telegram group: [@Gozargah_Flew](https://t.me/gozargah_flew)
- Ask on our [GitHub Discussions](https://github.com/gozargah/flew/discussions) for long term discussion or larger questions.
...
- Also tell the version of Flew, Xray and docker (if you use docker) you are using.
...
## Flew CLI
Flew CLI is built using [Typer](https://typer.tiangolo.com/), and its commands' code can be found in `cli`  directory. Its documentation is generated using [Typer CLI](https://typer.tiangolo.com/typer-cli/) which can be re-generated by navigating to project's root directory and running the following command (`typer-cli` package needs to be installed first):

```bash
$ PYTHONPATH=$(pwd) typer flew-cli.py utils docs --name "" --output ./cli/README.md
```
